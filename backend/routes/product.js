...
router.get('/', async (req, res) => {
  try {
    ...
    const products = await Product.find({}).exec(); // Changed title to name
    ...
    res.sendError(err, Utils.parseErrorString(err) || 'Error fetching products', 500);
  }
});
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    if (product) {
     ...
     // Use res.sendError with parsed message and status 500
    res.sendError(err, Utils.parseErrorString(err) || 'Error fetching product details', 500);
  }
});
